...
class Track_visitor {
     ...
     */
    private $track_visitor = "track_visitor";
    ...
    private function log_visitor() {
        if ($this->track_session() === TRUE) {
            ...
            if (isset($temp_current_page) && $temp_current_page != $current_page) {
                ...
                $this->ci->db->insert($this->track_visitor, $data);
            ...
            $result = $this->ci->db->insert($this->track_visitor, $data);
            if ($result === FALSE) {
                /**
                ...
                $this->ci->db->where('track_visitor_id', $entry_id);
                $data = array(
                    'no_of_visits' => $count
                );
                $this->ci->db->update($this->track_visitor, $data);
